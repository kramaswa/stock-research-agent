...
If condition 1 fails (Q4 = firm NO) → ceiling remains Hold regardless of moat quality.
...
**10-Year Compounder Math:** [only include this line when Signal is Add to Position via the Long-Horizon Compounder path] — show the explicit calculation: "At [X]% annual EPS growth ([source: consensus/guidance/stated assumption]), year-10 EPS reaches ~$[Y]. At [Z]x exit P/E (held at/below current forward P/E of [current fwd P/E]x), the stock reaches ~$[W] — approximately [N]x today's $[current_price], or ~[R]% annualized. This stock can potentially [N]X in 10 years at consensus growth rates."
