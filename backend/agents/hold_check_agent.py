...
def build_raw_metrics_block(raw: dict[str, Any]) -> str:
    ...
    return (
        ...
        "⚠ ADR / NEW US LISTING NOTE: For stocks that are newly listed US ADRs or new US exchange listings where the underlying company has traded on a foreign exchange for years (e.g., SK Hynix on KRX, ASML on Euronext, TSM on TWSE), Finnhub's return data reflects the UNDERLYING EXCHANGE price history — not the US ADR or US listing's actual trading history. When the analysis context or news indicates the stock is a new US ADR/listing and price returns appear very high (>100% over 26 or 52 weeks), explicitly note in the Pre-Check: 'These return figures reflect [company]'s performance on the [underlying exchange], not the US ADR holder's experience since US listing. A US investor who purchased the ADR at its US launch date has seen a different return profile.' The Q1 flag remains meaningful (the underlying business has genuinely appreciated), but do not imply that a US ADR investor is sitting on returns they could not have captured.\n\n"
        "⚠ ADR PREMIUM RISK (mandatory for new US ADRs): Newly listed US ADRs frequently trade at a premium to their underlying home-exchange shares — sometimes 15–50% or more — during the initial weeks or months of US trading. This premium can compress to zero at any time regardless of underlying business performance, representing pure structural downside for US ADR holders that is entirely separate from the business investment thesis. If the news analysis or any provided context mentions that the ADR trades at a premium to its home-exchange listing: (1) You MUST flag ADR premium compression explicitly in the Bear Case as a standalone risk: 'If the ADR premium of approximately X% compresses toward parity, this represents an additional [X]% downside from today's US price with no change in underlying business fundamentals.' (2) You MUST include it in Key Risks. (3) You MUST factor this structural downside into your Q4 (margin of safety) assessment — an ADR trading at a 30% premium to its home-exchange equivalent has no margin of safety from that premium alone. Do not treat ADR premium compression as a minor footnote; for a new US ADR with a meaningful premium, it is often the single largest near-term risk.\n\n"
