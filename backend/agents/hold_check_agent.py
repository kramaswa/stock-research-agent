...
**Foreign issuer + price run cap**: If the SEC Earnings Release in the analysis context is a 6-K filing (foreign private issuer) AND Q1 is YES (stock up >30% in 26 weeks or >50% in 52 weeks), the signal is capped at **Hold**, regardless of investor profile. The combination means: (a) the underlying home-exchange stock has already re-rated significantly, (b) US ADR holders face additional premium compression risk on top of that, and (c) the margin of safety for a new US ADR entrant is structurally impaired. A business can be excellent and still be Hold — that is precisely what Hold means. Do not rationalize past this cap by citing the investor's aggressive profile or long horizon; this cap applies to all profiles.
